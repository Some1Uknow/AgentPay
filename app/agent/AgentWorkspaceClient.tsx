...
function InlineRunProof({ run }: { run: AnyObj }) {
  ...
  return <div className="inline-proof-trail" aria-label="Execution proof">
    ...
    <ProofSection title="Validation writes">{(run?.validationUpdates || []).length ? run.validationUpdates.map((v: AnyObj) => <TxProof key={v.responseTxHash || v.requestHash} title={v.name} meta={`score ${v.score}/100`} hash={v.responseTxHash} />) : <p className="empty-copy">No validation write was created.</p>}</ProofSection>
