...
function InlineRunProof({ run }: { run: AnyObj }) {
  ...
  return <div className="inline-proof-trail" aria-label="Execution proof">
    ...
    <ProofSection title="ERC-8004 validation">{(run?.validationUpdates || []).length ? run.validationUpdates.map((v: AnyObj) => <TxProof key={v.responseTxHash || v.requestHash || `${v.agentId}-${v.name}`} title={v.name} meta={v.error ? 'validation attempted · reputation still recorded' : `score ${v.score}/100`} hash={v.responseTxHash} />) : <p className="empty-copy">No validation write was created.</p>}</ProofSection>
